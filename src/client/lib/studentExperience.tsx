...
function getProfileOptions(t: any): StudentProfileOption[] {
  return [
    {
      id: 'standard',
      label: t('accessibility.option.standard.label'),
      body: t('accessibility.option.standard.body'),
      icon: <Sparkles className="w-4 h-4" />,
    },
    {
      id: 'focus',
      label: t('accessibility.option.focus.label'),
      body: t('accessibility.option.focus.body'),
      icon: <Accessibility className="w-4 h-4" />,
    },
    {
      id: 'contrast',
      label: t('accessibility.option.contrast.label'),
      body: t('accessibility.option.contrast.body'),
      icon: <Contrast className="w-4 h-4" />,
    },
    {
      id: 'touch',
      label: t('accessibility.option.touch.label'),
      body: t('accessibility.option.touch.body'),
...
function StudentExperienceDock({
  ...
  const activeProfile = options.find((option) => option.id === profile) || options[0];

  return (
    <div 
      dir={direction}
      className={`fixed bottom-4 z-[90] flex flex-col-reverse gap-3 pointer-events-none ${isRtl ? 'left-4 items-start' : 'right-4 items-end'}`}
    >
      <button
        type="button"
      ...
      >
        ...
        <span className="hidden sm:inline">{t('accessibility.dock.label')}</span>
      </button>

      {open && (
        <div className="pointer-events-auto w-[min(92vw,360px)] max-h-[80vh] overflow-y-auto rounded-[2rem] border-4 border-brand-dark bg-white p-5 shadow-[10px_10px_0px_0px_#1A1A1A] thin-scrollbar">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div className={isRtl ? 'text-right' : 'text-left'}>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-purple mb-2">{t('accessibility.dock.title')}</p>
              <h3 className="text-2xl font-black text-brand-dark">{t('accessibility.dock.heading')}</h3>
              <p className="font-bold text-brand-dark/60 mt-2">
                {t('accessibility.dock.body')}
              </p>
            </div>
            <button
              type="button"
              ...
              aria-label={t('accessibility.dock.close')}
            >
              <X className="w-4 h-4" />
            </button>
          </div>
                  ...
                  <div className={`flex items-start justify-between gap-3 ${isRtl ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className="flex-1">
                      <div className={`flex items-center gap-2 mb-2 ${isRtl ? 'flex-row-reverse' : 'flex-row'}`}>
                        <span className="w-8 h-8 rounded-full border-2 border-brand-dark bg-white flex items-center justify-center shrink-0">
                          {option.icon}
                        </span>
                    ...
                    {isActive && <CheckCircle2 className="w-5 h-5 text-brand-orange shrink-0 mt-1" />}
                  </div>
                </button>
              );
            })}
          ...
          <div className={`mt-4 rounded-[1.3rem] border-2 border-brand-dark bg-brand-bg p-4 ${isRtl ? 'text-right' : 'text-left'}`}>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-brand-orange mb-2">{t('accessibility.dock.current')}</p>
